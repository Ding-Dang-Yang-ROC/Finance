...
def Train_Pyt(one,img):
  ...
  for i in range(n_batch):
    ...
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,8,2).permute(1,3,2,0).reshape(4,8*batch) # 04 123
    pred=torch.matmul(Con_Unitary(Par1[0],S_Ker1),img1).reshape(2,2,4,2,batch).permute(0,3,2,1,4).reshape(4,8*batch) # 03 124
    pred=torch.matmul(Con_Unitary(Par1[1],S_Ker1),pred).reshape(2,2,2,2,2,batch).permute(0,3,2,1,4,5).reshape(4,8*batch) # 02 134
    pred=torch.matmul(Con_Unitary(Par1[2],S_Ker1),pred).reshape(2,2,2,4,batch).permute(0,2,1,3,4).reshape(4,8*batch) # 01 234
    pred=torch.matmul(Con_Unitary(Par1[3],S_Ker1),pred).reshape(32,batch).permute(1,0).reshape(batch,2,16)
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range(n_batch):
    ...
    batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,8,2).permute(1,3,2,0).reshape(4,8*batch) # 04 123
    pred=torch.matmul(cm0,img1).reshape(2,2,4,2,batch).permute(0,3,2,1,4).reshape(4,8*batch) # 03 124
    pred=torch.matmul(cm1,pred).reshape(2,2,2,2,2,batch).permute(0,3,2,1,4,5).reshape(4,8*batch) # 02 134
    pred=torch.matmul(cm2,pred).reshape(2,2,2,4,batch).permute(0,2,1,3,4).reshape(4,8*batch) # 01 234
    pred=torch.matmul(cm3,pred).reshape(32,batch).permute(1,0).reshape(batch,2,16)
    pred=pred.pow(2).sum(dim=2)
...
def quantum_circuit(state,cm0,cm1,cm2,cm3):
  ...
  qml.QubitUnitary(cm0,wires=[0,4])
  qml.QubitUnitary(cm1,wires=[0,3])
  qml.QubitUnitary(cm2,wires=[0,2])
  qml.QubitUnitary(cm3,wires=[0,1])
